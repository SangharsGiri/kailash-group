...
class Flight extends MX_Controller{
    ...
    {
        $data['records'] = $this->flight->get_flight_list();
    ...
    public function form($id=0)
    {
        ...
        if($this->input->post())
        {
             ...
             $this->form_validation->set_rules('flight_title', 'flight_title','required' );
            // $this->form_validation->set_rules('flight_page_title', 'flight_page_title', 'required|callback_check_name_exist['.$this->input->post('flight_id').']');
            $this->form_validation->set_rules('flight_body', 'flight_body');
            $this->form_validation->set_rules('flight_type', 'flight_type');
            ...
            $new_tags = $this->input->post('flight_tags');
            $folder_path = '../uploads/flight/';
            $flight_id = $this->input->post('flight_id');
            $insert['flight_title'] = $this->input->post('flight_title') ;
            $insert['duration'] = $this->input->post('duration') ;
            $insert['flight_page_title'] = $this->input->post('flight_page_title');
            $insert['flight_body'] = $this->input->post('flight_body');
            ...
            $insert['flight_type'] = $this->input->post('flight_type');
            ...
            $insert['flight_url'] = strtolower(str_replace(" ","-",$insert['flight_page_title']));
           // $insert['parent_id'] = $this->input->post('parent_id');
            ...
            // $check_url =  $this->crud->check_url($flight_id, 'flight_id', $url, 'flight_url','igc_flight');
            // if(!empty($check_url))
            // {
            //     $insert['flight_url']= $url."-".rand(1, 50);
            // }
            // else
            // {
            //     $insert['flight_url'] = $url;
            // }
            ...
            if($insert['flight_type'] == "Page")
            {
                $insert['parent_id'] = $this->input->post('parent_id');
                $result = $this->flight->flight_insert($insert);
                //code to add tags
                ...
                $inserts['flight_id']= $result;


                $this->flight->tab_insert($inserts);

                if($result)
                {
                    //code to create log
                    $log['module_title']= $insert['flight_page_title'];
                    $log['action_id']= "1";
                    $this->create_log($log);

                    $this->session->set_flashdata('success','New flight has been inserted.');
                    redirect('flight');
                }
                else{
                    $this->session->set_flashdata('error','Unable to insert the new flight.');
                    redirect('flight');
                }
            }
            else{
                ...
                if($category == "")
                {
                    ...
                    $results = $this->flight->insert_flight_category($cat);
...
//                    print_r($insert);
//                    exit;
                    $result = $this->flight->flight_update($insert, $flight_id);
                    //code to add tags
                    $this->add_tags($flight_id, $new_tags);
                    $inserts['tab1'] = $this->input->post('tab1');
                    $inserts['description1'] = $this->input->post('description1');
                    $inserts['tab2'] = $this->input->post('tab2');
                    $inserts['description2'] = $this->input->post('description2');
                    ...
                    $tab_data = $this->flight->tab_detail($flight_id);

                    if(!empty($tab_data))
                    {
                        $this->flight->tab_update($inserts, $flight_id);
                    }
                    else{
                        $inserts['flight_id'] = $flight_id;
                        $this->flight->tab_insert($inserts);
                    }
                    ...
                    if($result)
                    {
                        ...
                        $log['action_id']= "2";
                        $this->create_log($log);
                        $this->session->set_flashdata('success','flight has been updated.');
                        redirect('flight');
                    }
                    else{
                        $this->session->set_flashdata('error','Unable to update the flight.');
                        redirect('flight');
                    }
                }
                else
        ...
        $flight = $this->flight->get_flight_detail($id);
        $tab = $this->flight->tab_detail($id);

        //$data['flight'] = array_merge($flight, $tab);

        $data['categories'] = $this->flight->get_flight_categories();
        $data['parent_pages'] = $this->flight->get_parent_page();


        $data['scripts'] = array('themes/js/form-validator');
        $data['title']= "Add/Edit Flight";
        $this->load->view('header', $data);
        $this->load->view('flight_form');
        $this->load->view('footer');
    }

    public function check_name_exist($str, $flight_id)
    {
        $string = str_replace(" ","-",$str);
        $name = $this->flight->check_name_exist($string, $flight_id);
        ...
        {
            $this->form_validation->set_message('check_name_exist', 'flight page title already exist.');
            return FALSE;
        }
    ...
    {
        $detail= $this->crud->get_detail($id, 'flight_id', 'igc_flight');
        if($detail['flight_type']=="Page")
        {
            redirect('flight');
        }
        $data['page_title'] = $detail['flight_page_title'];
        $data['records'] = $this->crud->get_detail_rows($id, 'flight_id', 'igc_flight_comments');
    ...
    public function comment_active()
    {
        if($this->input->post())
        {
            ...
            $this->crud->update($id, 'comment_id', $update, 'igc_flight_comments');
            //code to create log
            $log['module_title']= "flight Comment Id"."(".$id.")";
            $log['action_id']= "2";
            $this->create_log($log);
            echo "success";
        }
    ...
    {
        $result = $this->crud->delete($id,'comment_id','igc_flight_comments');
        if($result)
        {
            //code to create log
            $log['module_title']= "flight Comment Id"."(".$id.")";
    ...
    //code to soft delete the flight

    public function delete($flight_id)
    {
        $flight_detail = $this->crud->get_detail($flight_id, 'flight_id', 'igc_flight');
        $result = $this->flight->delete_flight($flight_id);
        if($result)
        {
            //code to create log
            $log['module_title']= $flight_detail['flight_page_title'];
            $log['action_id']= "3";
            $this->create_log($log);

            $path='../uploads/flight/';
            @unlink($path.$flight_detail['featured_img']);
            $this->session->set_flashdata('success','flight has been deleted.');
            redirect('flight');
        }
        else{
            $this->session->set_flashdata('error','Unable to delete the flight.');
            redirect('flight');
        }
    }
...
//code to add new tags in flight

    public function add_tags($flight_id, $tags)
    {
        if ($tags != '') {
            $tagIn = trim(preg_replace('/\s+/', ',', $tags));
            $tags = explode(',', $tagIn);

            $this->flight->insert_tags($tags, $flight_id);
        }

    }

    public function rmv_flight_tag()
    {
        if($this->input->post())
        {
            ...
            $this->db->delete('igc_flight_tags');
    ...
    public function send($flight_id)
    {
        $flight_detail = $this->crud->get_detail($flight_id, 'flight_id', 'igc_flight');
        $subscribes = $this->flight->get_subscribers();
        if(! empty($flight_detail) && $flight_detail['flight_type'] == 'Article' && ! empty($subscribes))
        {
            $site_settings = $this->site_settings->get_site_settings();
            $server_url = $site_settings['site_url'].'/index.php/blog/detail/'.$flight_detail['flight_url'];
