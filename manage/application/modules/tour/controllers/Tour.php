...
class Tour extends MX_Controller{
    ...
    {
        $data['records'] = $this->tour->get_tour_list();

        $data['title']= "tour List";
        $this->load->view('header', $data);
        $this->load->view('tour_list');
    ...
    public function form($id=0)
    {
        ...
        if($this->input->post())
        {
           ...
           // $insert['parent_id'] = $this->input->post('parent_id');
            ...
            {
                $insert['parent_id'] = $this->input->post('parent_id');
                $result = $this->content->tour_insert($insert);
                //code to add tags
...
//                    print_r($insert);
//                    exit;
                    $result = $this->content->tour_update($insert, $tour_id);
                    //code to add tags
                    ...
                    if(!empty($tab_data))
                    {
                        $this->content->tab_update($inserts, $tour_id);
                    }
                    else{
                ...
                else
                {
                    ...
                    $result = $this->content->content_update($insert, $tour_id);
                    if($result)
                    {
                        //code to create log
                        $log['module_title']= $insert['tour_page_title'];
        ...
        $tour = $this->tour->get_tour_detail($id);
        $tab = $this->tour->tab_detail($id);


        $data['categories'] = $this->tour->get_tour_categories();
        $data['parent_pages'] = $this->tour->get_parent_page();


        $data['scripts'] = array('themes/js/form-validator');
        $data['title']= "Add/Edit Tour";
        $this->load->view('header', $data);
        $this->load->view('tour_form');
        $this->load->view('footer');
    }

    public function check_name_exist($str, $tour_id)
    {
        $string = str_replace(" ","-",$str);
        $name = $this->tour->check_name_exist($string, $tour_id);
        ...
        {
            $this->form_validation->set_message('check_name_exist', 'tour page title already exist.');
            return FALSE;
        }
    ...
    {
        $detail= $this->crud->get_detail($id, 'tour_id', 'sg_tour');
        if($detail['tour_type']=="Page")
        {
            redirect('tour');
        }
        $data['page_title'] = $detail['tour_page_title'];
        $data['records'] = $this->crud->get_detail_rows($id, 'tour_id', 'sg_tour_comments');
    ...
    public function comment_active()
    {
        if($this->input->post())
        {
            ...
            $this->crud->update($id, 'comment_id', $update, 'sg_tour_comments');
            //code to create log
            $log['module_title']= "tour Comment Id"."(".$id.")";
            $log['action_id']= "2";
            $this->create_log($log);
            echo "success";
        }
    ...
    {
        $result = $this->crud->delete($id,'comment_id','sg_tour_comments');
        if($result)
        {
            //code to create log
            $log['module_title']= "tour Comment Id"."(".$id.")";
    ...
    {
        $tour_detail = $this->crud->get_detail($tour_id, 'tour_id', 'sg_tour');
        $result = $this->content->delete_tour($tour_id);
        if($result)
        {
            //code to create log
            $log['module_title']= $tour_detail['tour_page_title'];
            $log['action_id']= "3";
            $this->create_log($log);

            $path='../uploads/tour/';
            @unlink($path.$tour_detail['featured_img']);
            $this->session->set_flashdata('success','tour has been deleted.');
            redirect('tour');
        }
        else{
            $this->session->set_flashdata('error','Unable to delete the tour.');
            redirect('tour');
        }
    }
...
//code to add new tags in content
    ...
    public function add_tags($tour_id, $tags)
    {
        if ($tags != '') {
            $tagIn = trim(preg_replace('/\s+/', ',', $tags));
            $tags = explode(',', $tagIn);

            $this->content->insert_tags($tags, $tour_id);
        }

    }

    public function rmv_tour_tag()
    {
        if($this->input->post())
        {
    ...
    public function send($tour_id)
    {
        $tour_detail = $this->crud->get_detail($tour_id, 'tour_id', 'sg_tour');
        $subscribes = $this->content->get_subscribers();
        if(! empty($tour_detail) && $tour_detail['tour_type'] == 'Article' && ! empty($subscribes))
        {
            $site_settings = $this->site_settings->get_site_settings();
            $server_url = $site_settings['site_url'].'/index.php/blog/detail/'.$tour_detail['tour_url'];
